...
extractComponentGeneSet <- function(seurat_obj,
  ...
  pca_obj <- ?  # <-- How do we extract this data?
  
  pulled_column <- pca_obj[pc_number]
  
  if (?) {  # <-- Fill in the conditional.
    result <- arrange(pca_obj, pca_obj[pc_number])
  } else {
    result <- arrange(pca_obj, desc(pca_obj[pc_number]))
  }
  
  result %>%
    ?(gene_total) %>%  # <-- Which function?
    rownames()
}

result <- extractComponentGeneSet(pbmc)
