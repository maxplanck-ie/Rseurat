...
```{r pre-setup, include=FALSE, eval=FALSE}
# Needs to be run only once.
# If asked to update packages, answer 'none'. We take care of it later.
...
# if (! "circlize" %in% installed_pkgs) install.packages("circlize")
# if (! "janitor" %in% installed_pkgs) install.packages("janitor")
# if (! "scCustomize" %in% installed_pkgs) remotes::install_github('samuel-marsh/scCustomize')
# if (! "glmGamPoi" %in% installed_pkgs) BiocManager::install("glmGamPoi")
```
...
# Cleared out: "SeuratDisk", "LoomExperiment", "MAST", "glmGamPoi", "scCustomize", "Nebulosa"

#reticulate::use_virtualenv(params$pyvenv_umap)
```
...
> HOMEWORK: check this beautiful explanation of permutations explained in the context of statistical testing (constructing a 'null distribution'): <https://www.jwilber.me/permutationtest/>
...
```{r}
Ndim=10
# HOMEWORK: comment this line and convert this into a parameter of the current Rmarkdown document.
```
...
- Did you just use a string to hardcode the path? How about adding the date inside the filename? E.g. `./RseuratCourseRootDir/config/Session_Tue_Mar__7_13-58-08_2023.log`

Hint: `gsub(' ','_',gsub(':','-', date())` will get you the date in the format we just showed, using dashes and underscores for safety (POSIX principles?). Also, be sure to use `file.path()`, `getwd()`, `paste0()`, etc. to make it look fancy, and get around all the awkwardness and rough edges... e.g. Does `file.path` add or lack a trailing `/`? If we use `getwd` but this is run inside the notebook subfolder `./rmd/`, don't we need to append a `..`? Check the output of `getwd()` interactively and on the rendered markdown document (we did run this command as the 2nd code block in this document just for this very reason :P)

- Implement remaining params: dimensionality of the dataset, and cluster resolution.

- Implement an overwrite of the knit function so that you save the HTML report somewhere else in the project folder.
