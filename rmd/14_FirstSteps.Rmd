...
And, we can have a heatmap of the first 1k genes and 1k cells:

```{r explore2}
pheatmap::pheatmap(log10(pbmc.data[1:1000, 1:1000]+1),
                   show_rownames = F,
                   show_colnames = F,
                   cluster_rows = F,
                   cluster_cols = F,
...
> 1. How many cells have less than 1000 counts in total (summed over all genes)? Hint: subset cells with `colSums()`.
> 1. How many genes are not expressed in any cell? Hint: subset genes with `rowSums()`.
> 1. Which are the top 3 genes with a higher row-wise mean? Hint: aggregate with `apply(dgCMatrix, 1, FUN)`.
> 1. Plot the histogram of counts for the previous top 3 genes.
