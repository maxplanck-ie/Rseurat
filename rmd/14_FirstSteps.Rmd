...
The raw data can be found [here](https://cf.10xgenomics.com/samples/cell/pbmc3k/pbmc3k_filtered_gene_bc_matrices.tar.gz), and you have already downloaded it with the repository Zip file. This dataset consists of 3 files: 
...
> 🧭✨ Polls:
> 1. How many genes and cells does this dataset have?
> 2. How many genes are not expressed in any cell? Hint: subset genes with `rowSums()`.
> 3. Which are the top 3 genes with the highest total count? Hint: `rowSums()`  and `sort`.
...
> 🧭✨ Polls In cell "AAATTCGATTCTCA-1", how many reads map to gene "ACTB"? Hint: subset the assay data, `pbmc@assays$RNA`.
...
On the other hand, `DimReduc` objects represent transformations of the data contained within the Assay object(s) via various dimensional reduction techniques such as PCA. For class-specific details, including more in depth description of the slots, please see the wiki sections for each class:
...
One of our first goals is to identify (and filter) dead cells that could be the results
of a harsh experimental protocol. A few QC metrics commonly used, include:
...
> > ⌨🔥 Poll:  How many cells have less than 200 counts in total (summed over all genes)?

Individually these variables may not fully discriminate dead cells, but could also 
reflect real biological properties (e.g. higher mitochondrial count). Therefore it is useful
to look a relationship between these variables.
...
> 🧭✨ Polls:
> 1. Which are the 10 most highly variable genes? Hint: use `VariableFeatures()`.
> 1. What's the variance of the gene `PYCARD`? hint: use `HVFInfo()`.
...
> 🧭✨ Poll: Imagine we pick a pair of cells, one right next to the other, that have an absolute distance on the X-axis of two units (the distance on Y-axis is null). Then, we pick another different pair of cells, one on top of the other; we measure their distance on the Y-axis, and it's again of two units (the distance on the X-axis is null). So, both have a distance of two units, but the first pair does so in X-axis and the other on the Y-axis. Which pair of cells is more divergent between themselves?
...
> 🧭✨ Poll: Which is the default value of the parameter `K` that Seurat uses? Hint: `help` yourselves by reading the docs!
