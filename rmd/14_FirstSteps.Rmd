...
POLL: How many genes and cells does this dataset have?
...
POLL: In cell "AAATTCGATTCTCA-1", how many reads map to gene "ACTB"? <!-- Hint: subset the sparse matrix in pbmc@assays$RNA -->
...
POLL: Which function, also present in base-R and/ or dplyr, would you use: select, filter, or subset?

POLL: What's the current number of cells after this step? 2583, 2638, 2385?
...
POLL: Which are the 10 most highly variable genes? Hint: use `VariableFeatures()`.
...
POLL: What's the variance of the gene `PYCARD`? hint: use `HVFInfo()` over your `assay`.
