...
# Seurat relies on Future Framework for speeding up computations
...
We will be analyzing a dataset of Peripheral Blood Mononuclear Cells (PBMC) freely available from 10X Genomics. There are around 3000 single cells that were sequenced on the Illumina NextSeq 500. The raw data can be found [here](https://cf.10xgenomics.com/samples/cell/pbmc3k/pbmc3k_filtered_gene_bc_matrices.tar.gz), and you have already downloaded it with the repository Zip file.
...
> ⌨🔥 Exercise: How many genes and cells does this dataset have?
...
> ⌨🔥 Exercise: In cell "AAATTCGATTCTCA-1", how many reads map to gene "ACTB"

> ⌨🔥 Exercise: Which are the top 3 genes with a higher row-wise mean? Hint: `apply(dgCMatrix, 1, FUN)`.


# Create Seurat Object

Initialize the Seurat object with the raw count data (and some filtering)
...
> ⌨🔥 Exercise: What is the dimensionalty of `pbmc.data` and `pbmc`?

The `min.cells` and `min.features` arguments are low-strigency filters. We are only loading cells with at least 200 genes detected, and we are only including those genes (features) that were detected in at least 3 cells. With these filters in this particular dataset, we are only reducing the number of genes from `33` to `14` thousands.
...
> ⌨🔥 Exercise: In cell "AAATTCGATTCTCA-1", how many reads map to gene "ACTB"? <!-- Hint: subset the sparse matrix in pbmc@assays$RNA -->
...
`FeatureScatter()` is typically used to visualize relationships between features, but can be used for anything calculated at the object, i.e. columns in object metadata, PC scores etc.
...
> ⌨🔥 Exercise: Which function, also present in base-R and/ or dplyr, would you use: select, filter, or subset?

> ⌨🔥 Exercise: What's the current number of cells after this step? 2583, 2638, 2385?
...
> ⌨🔥 Exercise: Which are the 10 most highly variable genes? Hint: use `VariableFeatures()`.
...
> ⌨🔥 Exercise: What's the variance of the gene `PYCARD`? hint: use `HVFInfo()` over your `assay`.
...
> ⌨🔥 Exercise: How many components should we choose to include? 10? 20? 100?
...
The next three code-block are skipped of evaluation because this process can take a long time for big datasets. Please note that there's nothing wrong with using `ElbowPlot()`!
...
```{r, echo=FALSE}
knitr::include_graphics("./images/jackstraw.png")
```
...
> ⌨🔥 Exercise: Convert this `Ndim` into a parameter at the current Rmarkdown document YAML header, just like `Nthreads`.
...
> ⌨🔥 Exercise: Machine learning algorithms have parameters and hyperparameters, Deep Learning Large Language Models have them in the order of billions, this one only has one: `K`. Can you find which is the default value that Seurat uses? 10? 20? 30?... Hint: `help` yourselves by reading the docs!

```{r}
Ndims=10
pbmc <- FindNeighbors(pbmc, dims = seq_len(Ndims))
```
...
> ⌨🔥 Exercise: How many clusters did we find?
...
> ⌨🔥 Exercise(s): These are **not mandatory**, and barely related to scRNAseq... we hope it will add to your 'Working with RMarkdown' skills. We'll start with these today, but we'll still need to allow for 30-45 minutes before the end of this class for one last lecture (titled: Enrichment).
...
> Hint: `format(Sys.time(), '%d_%B__%H-%M-%S_%Y')` will get you the date in the format we just showed, using dashes and underscores for POSIX-compliance/ safety.
> 
> Extra points if you use `file.path()`, `getwd()`, `paste0()`, etc. to make it look fancy. WARNING: There are rough edges... e.g. Does `file.path` add, or lack, a trailing slash? `/`? If we use `getwd` but this is run inside the notebook subfolder `./rmd/`, don't we need to append a `..`? Check the output of `getwd()` interactively and on the rendered markdown document (in fact, we added this command as the 2nd code block in this document just for this very reason :P)
...
>   '../lectures', paste0(format(Sys.time(),
>   '%d_%B__%H-%M-%S_%Y')), '_results.html')))})
> ```
> 
...
6. Confirm that you used the correct function (we asked for RDS file!), by reading the data into a new R session using the function `readRDS` and saving it in a variable with a new unused name. (You may remove variables with `rm()` if you want to keep the global environment tidy).

7. Re-run this analysis with another set of parameters. See that files are not overwritten, and that you can track which RDS, Session log, and HTML Report corresponds to each other; in each reiteration.
