...
**Polls**: How many genes and cells does this data set have?
...
The `SeuratObject` serves as a container that will contains both data (like the count matrix) and analysis (like PCA, or clustering results) for a single-cell dataset. For example, the count matrix is stored in `pbmc[["RNA"]]@counts`. On RStudio, you can use `View(pbmc)` to inspect all the slots.
...
The `VlnPlot()` function plots the probability density function for all specified variables (features).
...
## Cell Selection (subset)

Based on the cell-specific features we can subset the cells even further.

**Task**: Using the SeuratObject::subset() command, filter out cells
with less than 200 or more than 2500 counts, and more than 5% mitochondrial counts (Why?)
...
**Poll:** What's the number of cells after filtering? (2583, 2638, 2385)?
...
Next, we look at a subset of genes that exhibit high cell-to-cell variation in the dataset (i.e, they are highly expressed in some cells, and lowly expressed in others). To identify highly variable genes, Seurat models the mean-variance relationship inherent in the data using the `FindVariableFeatures()` function.
...
**Task**: Extract the 10 most highly variable genes using the function VariableFeatures()
