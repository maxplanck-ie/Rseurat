...
- Prepare the data for dimensional reduction (filter, transformations, ...)
...
```{r get_mt}
loo[["percent.mt"]] <- PercentageFeatureSet(loo, pattern = "^mt-")
loo@meta.data %>% head()
VlnPlot(
  loo,
...
```{r loo_prep}
loo_alone <- loo %>% 
  subset(nFeature_RNA > 200 & nFeature_RNA < 3000 & percent.mt < 5) %>%
  mySCT() %>% 
