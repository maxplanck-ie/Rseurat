...
**Poll** Which cell population has the highest total counts? Hint: you can make use of the `FetchData` function to calculated median RNA counts per cell population. Dplyr functions `summarize` and `group_by` could be helpful to summarize a numeric variable over factor levels.
...
**Poll** What amount of standard deviation does PC1 explain after sc-transform? Hint: if you'd like to determine it numerically, you can use the `Stdev` accessor function to access the standard deviations for PC components in a Seurat object.
...
Apply SCTransform to the IfnB-stimulated dataset as well. 
...
FeaturePlot(immune.combined.sct, features = inf_genes[1], split.by = "stim",
