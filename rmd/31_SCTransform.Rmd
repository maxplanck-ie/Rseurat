...
where $\theta_g$ is the gene-specific dispersion parameter, $n_{c}=\sum _{g} x_{gc}$ is the total sequencing depth of cell $c$ and the variance of the negative binomial (NB) is given by $ \mu_{gc} + \mu_{gc}^{2}/\theta_{g} $
...
\mu_{gc} &= \exp{\beta_{g0} + \ln{n_{c}}} \\ 
\sigma_{gc} &= \sqrt{\mu_{gc} + \frac{\mu_{gc}^{2}}{\theta_{g}}}, 
\end{array} 
$$
...
$$
\begin{array}{*{20}l} 
\mu_{gc} &= \exp{\beta_{g0} + \ln{n_{0}}}, \\ 
\sigma_{gc} &= \sqrt{\mu_{gc} + \frac{\mu_{gc}^{2}}{\theta_{gc}}},\\ 
y_{gc} &= \text{floor}(Z_{gc} \sigma_{gc} + \mu_{gc}), 
\end{array} 
$$
...
IFNB-Stimulated and human controls.
...
**Poll:** How does the lowD representation change using SCT versus log-Norm ?
