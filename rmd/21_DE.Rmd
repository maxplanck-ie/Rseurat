...
process_markers <- function(markers) {
  ...
  markers <- markers %>% filter(Adjusted.P.value < 5e-2)
  ...
  if ("cluster" %in% colnames(markers)) {
    ...
    markers <- markers %>% arrange(Adjusted.P.value) %>% select(
...
if ("Adjusted.P.value" %in% names(markers.between.clusters)) {
  ...
  markers.between.clusters <- markers.between.clusters %>% process_markers()
}
```
