...
my_load_namespaces(
  packagesToLoad = c("openxlsx", "readr", "magrittr", "dplyr", "RColorBrewer", "ggplot2", "patchwork", "scCustomize", "Seurat"),
  coreLibraries = c("knitr", "rmarkdown", "formatR", "DT", "reticulate", "sessioninfo", "limma", "DESeq2", "enrichR", "future", "uwot", "future.apply", "metap", "shiny", "SingleCellExperiment", "remotes", "bookdown", "SeuratData", "Nebulosa")
)
```
...
```{r}
my.file.name <- "25_Viz"
sessioninfo::session_info(to_file = file.path(getwd(), paste0("../", my.file.name, ".session.log")))
```
...
EXERCISE: Are there any differences in the results? If so, did you reset the seed between runs? What about the time it takes? You can measure with `microbenchmark` package.
