...
>
> According to the docs, perplexity should always follow: `3 * perplexity < nrow(X) -1`. Are the rows referring to genes or cells? Hint: see description of the matrix `X` in the help of `Rtsne()`... 
...
> ⌨🔥 Exercise: For UMAP, an important parameter is the number of neighbors. Play around with a couple of values!

> 🧭✨ Polls:
> 
> Run the UMAP without seed a couple of times and compare. Is it more affected than t-SNE? Or less? Hint: use the `seed.use` argument.
...
> ⌨🔥 Exercise: Distribute the room in groups, and run the algorithms with 2, 5, 8, 12, and/ or 15 PCs. Strive to avoid the aforementioned common pitfalls, specially the third one!

> 🧭✨ Polls:
>
> Would decreasing the number of PCs fed onto the clustering algorithm change our visualization? Would you say the results are 'better'?
>
