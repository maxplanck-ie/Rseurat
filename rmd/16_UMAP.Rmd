...
# extract CELL-PC matrix, transpose, and plot
...
```{r, echo=FALSE, out.width="49%", out.height="49%", fig.show='hold', fig.align="center", fig.cap="Consider a simple geographic map, it shares the same goal. Given its limits, it requires some choices: a sphere cannot be mapped uniquely into 2D. That is why we have different projections, depicted here is the Universal Mercator Projection. Each of the 6-degree UTM is the projection on a plane using the geoid shape of our planet."}
...
For single-cell studies, two methodologies are popular: t-SNE and UMAP. The second one was developed 10 years later, and it adds a bunch of advantages.


## t-SNE: t-distributed stochastic neighbor embedding

Here the key idea is to first assign distances between cells/ samples, $i$ and $j$, to probabilities
...
Notice that there are many parameters (with default values) that will affect the visualization.  The most important are: input data and its dimensionality, a seed variable and the **perplexity** (see `?Rtsne::Rtsne` for further details on this.)

> ⌨🔥 Exercise: Find out which parameters are used per default. Explore some parameter choices together with your classmates and observe if this will affect the plot. 
...
t-SNE and UMAP have the same principle and workflow: create a high dimensional graph, then reconstruct it in a lower dimensional space while retaining the structure. While t-SNE moves the high dimensional graph to a lower dimensional space points by points; UMAP compresses that graph. For further details, see some of these resources:
...
UMAP has a similar goal as t-SNE; but it also tries to preserve more global aspects of the data structure. It has several advantages:
...
> ⌨🔥 Exercise(s):
> 
> 1. Run the UMAP without seed a couple of times and compare. Is it more affected than t-SNE? Or less? Hint: add `seed.use = NULL` parameter.
> 
> 2. For UMAP, an important parameter is the number of neighbors. Play around with a couple of values!

<!-- Options/ Answers for Q1 up here ^^^^
- More. The plot has changed drastically.
- Less.  Due to the increased emphasis on global structure.
- Equal. Both tSNE and UMAP are sensitive on a similar level.
-->
...
1. You should look to more than one plot, changing its hyperparameters.

# End

> ⌨🔥 Exercise:
>
> 1. Would decreasing the number of PCs fed onto the clustering algorithm change our visualization? Would you say the results are 'better'? Distribute the room in groups, and run the algorithms with 2, 5, 8, 12, and/ or 15 PCs. Strive to avoid the aforementioned common pitfalls, specially the third one!
>
> 2. Could you have the UMAP projection onto 3 axis instead of 2?
