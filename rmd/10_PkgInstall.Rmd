...
```{r setup}
knitr::opts_chunk$set(eval=FALSE, format=TRUE, echo=FALSE)
```
...
The next code block only needs to be run **once**. If asked to update packages, answer `none`. We'll take care of it later.
...
If you see errors, re-run the code block again. Actually, notice the conditionals, you may re-run this code block as many times as you would like, until no package installation is run by it. The same principle applies to all further code blocks.

# Bioconductor

With `BiocManager`, we can install Bioconductor `3.14`. If you are using R 4.2 or 4.3, then you'd be looking for a newest release, see the [official release announcements](https://bioconductor.org/about/release-announcements/) to find your matching version.
...
utils::download.file("https://zenodo.org/record/7848230/files/panc_sub.RDS?download=1",destfile="datasets/preprocessed_rds/panc_sub.RDS",method="auto")}
if (!file.exists("datasets/preprocessed_rds/ref.labels.RDS")){
utils::download.file("https://zenodo.org/record/7845337/files/ref.labels.RDS?download=1",destfile="datasets/preprocessed_rds/ref.labels.RDS",method="auto")}
if (!file.exists("datasets/preprocessed_rds/pbmc_ref.seurat.RDS")){
utils::download.file("https://zenodo.org/record/7845337/files/pbmc_ref.seurat.RDS?download=1",destfile="datasets/preprocessed_rds/pbmc_ref.seurat.RDS",method="auto")}
```
