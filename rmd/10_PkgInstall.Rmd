...
Make sure you allow for ~45 minutes to complete all the steps. The exact time will depend on network performance, and the current state of your package library (e.g., previous packages installed). If all goes well, it will be less than 15 minutes.
...
The package installation may be accelerated A LOT by executing the following shell command in a Terminal **inside the server** (on RStudio IDE, you can open this using the 'Tools' menu).
