...
```{r SeuratData, eval=FALSE, echo=FALSE}
# This was used during pre-processing
library(SeuratData)
...
```{r full_umap, eval=FALSE, echo = FALSE,fig.width=12,fig.show='hold',fig.align='center',fig.cap=c("By technology.","By cell type.")}
# This code was run to produce the figure
my_cols = brewer.pal(panc@meta.data$tech %>% unique() %>% length(),"Dark2")
...
ag <- rownames(panc)                
vg <- VariableFeatures(panc)

panc <- panc %>%
  ScaleData(features=ag)  %>%
  RunPCA(features=vg)  %>%
  RunUMAP(dim=1:30)

p1<-DimPlot(panc, group.by="tech", cols=scales::alpha(my_cols,0.3))
p2<-DimPlot(panc, group.by="celltype")
p1+p2
ggplot2::ggsave("images/panc8full.umap.png",width=12,height=5)
```
...
For faster processing, we have prepared a subsampled dataset which contains only cells from three experiments
```{r create_subset, eval=FALSE, echo=TRUE}
panc_sub <- subset(x=panc,subset = dataset == c("celseq","celseq2","smartseq2"))
saveRDS(panc_sub,"panc_sub.RDS")
```
...
d <- GetAssayData(panc_sub) %>% as.matrix() %>% t() 
...
# explain batch.estimate object (what is obs and exp.)
...
```{r kBET3_plot}
FeaturePlot(panc_sub,features="PC_1")
...
## Seurat Integration: joint dim reduction
...
## Seurat Integration: cellular neighbourhoods
...
## Seurat Integration: robust anchors
