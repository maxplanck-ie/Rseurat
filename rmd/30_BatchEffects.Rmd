...
multinom(as.factor(b) ~ pe[,1])$AIC
multinom(as.factor(b) ~ pe[,2])$AIC
multinom(as.factor(b) ~ pe[,3])$AIC
```
...
It is apparent that PC5 differentiates cells sequenced with smartseq2 technology from those sequenced with celseq2.    
...
par(mfrow=c(1,2))
hist(before$r2[1:50,"R.squared"],xlim=c(0,1),breaks=100,main=sprintf("Before batch \n effect removal"),xlab="R.squared")
hist(after$r2[1:50,"R.squared"],xlim=c(0,1),breaks=100,main=sprintf("After batch \n effect removal"),xlab="R.squared")
par(mfrow=c(1,1))
