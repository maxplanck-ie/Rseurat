...
For details and results consult the papers. The only steps given are:

1. setups, libraries and convenience functions.
2. data loading

Please make sure to run the two code blocks below!
It is easy, but in the real world this may take a substantial amount of time.
...
myNorm <- function(x) {
  # Normalizes Seurat object and finds Variable Features
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 2000)
}
 
myScale <- function(x, int_features) {
  # Scales Seurat Object and runs PCA
...
# runs ~5min
dir <- "/scratch/local/hohl/rcourse-dat/mouse_brain/"
...
## Old integration
... not executed
```{r old_integration, eval=FALSE, echo=FALSE}
# from Tobi
sl <- c(loo, db)                              # list of individual datasets

sl <- lapply(sl, myNorm)                      # indiviudal norm and variable features
int_features <- SelectIntegrationFeatures(sl) # select variable genes across datasets
sl <- lapply(sl, myScale, int_features)       # individual scale and PCA using int_features 

anchors <- FindIntegrationAnchors(sl, anchor.features = int_features, reduction = 'rpca')
...
int <- ScaleData(int, verbose = vf)      # why is SCTransform() wrong here?
