...
# Setup
```{r knitr_opts}
# Do _NOT_ run any code block below
...
```{r}
# choice of name for subset is totally yours
loo_alone <- ....
```
...
loo_alone@meta.data %>% select(c("Cluster","__your_choice_here")) %>% table() %>% pheatmap()     # compare cluster solution with previous annotation
loo_alone %>% subset(idents=__your_choice_here) %>% DimPlot()                                    # cluster selection
```
