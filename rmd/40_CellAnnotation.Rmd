...
library(kableExtra)
library(magrittr)
knitr::opts_chunk$set(fig.width=6, fig.height=4,message=FALSE,warning=FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=70),eval=FALSE) 
```
...
pbmc_ref.seurat_sub<-subset(pbmc_ref.seurat,downsample=1000)
pbmc_ref.seurat_sub_diet<-DietSeurat(pbmc_ref.seurat_sub,counts=FALSE,scale.data=FALSE,dimreducs="pca")
```

## Seurat Integration: transfer labels

```{r transfer_data}
pbmc_sub<-subset(pbmc,downsample=1000)
pbmc_diet<-DietSeurat(pbmc_sub,counts=FALSE,scale.data=FALSE,dimreducs="pca")
p1<-profmem({
pbmc.anchors <- FindTransferAnchors(reference = pbmc_ref.seurat_sub_diet, query = pbmc_diet, dims = 1:30,reduction="cca")}) #>19Gb
#reference.reduction = "pca",reduction="pcaproject"#>19Gb; retained 4 anchors
#reduction="cca" > 7Gb, retained 2331 anchors

predictions <- TransferData(anchorset = pbmc.anchors, refdata = pbmc_ref.seurat_sub_diet$label,dims = 1:30)
pbmc<-pbmc_diet
