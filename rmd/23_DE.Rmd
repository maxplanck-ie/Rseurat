...
- Try the `markers_within_clusters()` function with the above.
...
- How about rewriting the 'treatment' metadata, but ensuring there's a 1:1 relationship between treatments in each of the clusters? Hint: you'd need to apply `sample` for each cluster as a subset. This exercise is not purely theoretical, having the skillset to test your assumptions with code is one of the greatest strengths you can come by.

- Re-do with a 1:20 relationship, and run downstream 'Within Clusters' DE analyses. How are the results affected?
